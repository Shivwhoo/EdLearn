...
/**
 * /vision-board — a student's private board of learning, career and personal
 * goals. Authenticated only: the guard mirrors /dashboard (store token with a
 * localStorage fallback, redirect to /login when missing), and the API itself
 * scopes every row to the verified user, so the page can never render someone
 * else's visions.
 */
...
export default function VisionBoardPage() {
  ...
  // Protected route: no token means straight to /login.
  useEffect(() => {
    if (isMounted && !authToken) router.push('/login');
  }, [isMounted, authToken, router]);
  ...
  // Pre-auth / pre-hydration guard, matching the dashboard's behaviour.
      ...
      <main className="relative min-h-screen bg-gradient-to-b from-blue-50 via-slate-50 to-slate-50 p-6 pt-24 md:ml-60 md:p-12 md:pt-12">
        ...
        <div className="relative z-10 mx-auto max-w-7xl space-y-8">
          ...
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-center md:justify-between">
            ...
            <button
              onClick={openCreate}
              className="flex items-center gap-1.5 self-start rounded-xl bg-blue-600 px-4.5 py-2.5 text-xs font-semibold tracking-[0.01em] text-white shadow-lg shadow-blue-600/20 transition-all hover:bg-blue-700 hover:shadow-blue-600/30 active:scale-95 md:self-center cursor-pointer"
            >
